...
> 
> 相比原版删除了下标运算符/数组访问的内容，增加了一些与标准 Python 不同之处的提示。
...
本工程意在使用 C++ 程序模拟 Python 的解释器模式，生成的可执行文件期望输入为一个完整的，在我们规定的语法规则下**保证词法与语法上合法，无未定义行为，不产生运行时错误**的 Simplified Python 程序，期望输出为这个程序的输出内容。工程的主题部分已经使用 Antlr4 工具完成建构，作为一项工程作业，你被期望填充最重要的控制流结构与模拟部分，
...
本工程 Python 程序的合法字符集是 ASCII 字符集的一部分。
...
尝试让标识符的长度超过64个字符是未定义行为。<font color=yellow>请注意此处弱于标准 Python 行为。</font>
...
- 字符串字面量：由成对的双引号 `"` 或单引号 `'` 所包围的字符串。
  ...
  - 检测到前一个引号后，其到下一个同类型引号之间的内容就是字符串的内容。比如 `"'12''"` 的内容是 `'12''` （或者在 C++ 中表示为 `"\'12\'\'"`）
  
  - 字符串中的字符一定是可显示字符（printable character）或空格。这意味着你不需要处理任何转义字符（`\", \', \\, \n, \t` 等）<font color=red>存疑，待检查</font>
...
- 格式化字符串字面量（format string literal / f-string literal）：由一个 `f"` 开头，到下一个非 `f"` 里的 `"` 结尾的部分构成一个格式化字符串。
  ...
  - 与字符串不同，其内部允许用一对 `{}` 括起一个表达式，在解析格式化字符串时将这个表达式求值，并将其格式化为字符串形式替代这个 `{}` 所占的部分。例如：
...
- `str`：字符串，<font color=red>不可变</font>，类似 C++ 的 `std::string`。
...
- `tuple`：元组，可以按组存储一系列数据，类似 C++ 的 `std::tuple`。
  ...
  - 让一个具名变量存储元组是未定义行为，显式构造元组也是未定义行为。这意味着元组长度可以是不小于 2 的正整数。<font color=yellow>请注意此处弱于标准 Python 行为。</font>
...
如果在当前语境下，没有存活的名称为 `var` 的变量，则该变量会被即时定义，开始其生命周期。如果存在生命周期未完的 `var` 变量，则它指向的就是这个变量。
...
2. 从左到右进行赋值，即
   ...
   > 
   > 不过呢 Simplified Python 里面不涉及这样的复杂设计，不会出现这样的事情。即使出现了也不影响你实现：使用遍历 AST 节点的解释器模式进行计算，这种特殊情形反而是实现难度更简单的。
...
一个表达式也可以独立成为一个语句。一般来说这个表达式是一个无返回值（返回值为 `None`）的函数，希望让这个函数发挥其作用。当然它也可以是个有其他返回值的表达式，不过这个值会被立刻丢弃。
...
`elif` 类似 C++ 的 `else if`，它的优势在于可以避免增加缩进层数。
...
- `continue` 用于结束本次 `while` 循环，直接开始下一个 `while` 循环的 `condition` 判定。（保证它在一个 `while` 循环内） 

- `return` 可以接一个表达式（可能用逗号组合为元组），比如 `return a` 或 `return 1, 3, b`； 也可以不接表达式，等效于 `return None`。它用于结束本层函数调用并给出一个返回值。（保证它出现在一个函数的定义内）
...
- 有些变量可以有默认值，但是都必须出现在无默认值的变量后面。（这点是为了解释器消歧义用的，语法规则文件里没有提但是 Antlr4 应当已经做了检查。）
...
- 参数有两种形式：关键字参数（keyword argument）和位置参数（positional argument）。我们以 `def foo(a, b)` 为例：
  
  - 关键字参数：即显式指定将参数传递给特定变量。比如 `foo(b = 2, a = 1)` 表示传入参数 `a` 的值为 `1`，`b` 的值为 `2`。
  
  - Positional argument 即按照函数定义时出现的参数顺序传值。比如 `foo(1, 2)` 表示传入第一个参数 `a` 的值为 `1` ，第二个参数 `b` 的值为 `2`。 
  
  - 若一个参数列表中同时有两种参数形式，那么位置参数必须出现在所有关键字参数之前，如 `foo(1, b=2)`。其设计意图是为了消歧义。
  
  - 每一个函数定义中未赋默认值的参数都应该以两种关键字参数之一的形式赋值，未能做到此条的函数调用会产生运行时错误。
...
- `bool`：单参数函数， 将 `int` 或 `float` 或 `str` 或 `None` 转成 `bool`。
  ...
  - 转换 `int, float` 的行为同 `static_cast<bool>`。
